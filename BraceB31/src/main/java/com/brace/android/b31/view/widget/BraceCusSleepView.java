...
public class BraceCusSleepView extends View {

    private static final String TAG = "BraceCusSleepView";
    //深睡颜色
    ...
    public BraceCusSleepView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initAttr(context,attrs);
    }

    public BraceCusSleepView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttr(context,attrs);
    }

    private void initAttr(Context context, AttributeSet attrs) {
        TypedArray typedArray =context.obtainStyledAttributes(attrs, R.styleable.BraceCusSleepView);
        if(typedArray != null){
            hightSleepColor = typedArray.getColor(R.styleable.BraceCusSleepView_lightSleepColor,0);
            deepSleepColor = typedArray.getColor(R.styleable.BraceCusSleepView_deepSleepColor,0);
            awakeSleepColor = typedArray.getColor(R.styleable.BraceCusSleepView_awakeSleepColor,0);
            sleepHeight = typedArray.getDimension(R.styleable.BraceCusSleepView_sleepViewHeight,MiscUtil.dipToPx(context,180));
            sleepEmptyData = typedArray.getDimensionPixelSize(R.styleable.BraceCusSleepView_sleepEmptyData,dp2px(15));
            noDataColor = typedArray.getColor(R.styleable.BraceCusSleepView_b30SleepNoDataColor,Color.parseColor("#6074BF"));
            insomniaColor = typedArray.getColor(R.styleable.BraceCusSleepView_b31sInsomniaColor,Color.YELLOW);
            deepAndLightColor = typedArray.getColor(R.styleable.BraceCusSleepView_b31sDeepAndLightColor,Color.parseColor("#15E090"));
    ...
    public void drawEmptyTxt(Canvas canvas) {
        if(sleepList == null || sleepList.size()<=0){
            canvas.drawText(getResources().getString(R.string.nodata),width/2-(getTextWidth(emptyPaint,getResources().getString(R.string.nodata))/2),-getHeight()/2,emptyPaint);
        }
