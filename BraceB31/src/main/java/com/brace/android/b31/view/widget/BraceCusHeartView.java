...
/**
 * 心率图表，每半小时一个点，一天最多48个点
 * Created by Administrator on 2018/8/1.
 */

public class BraceCusHeartView extends View {

    private static final String TAG = "BraceCusHeartView";
    ...
    public BraceCusHeartView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
    }

    public BraceCusHeartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BraceCusHeartView);
        if (typedArray != null) {
            height = typedArray.getDimension(R.styleable.BraceCusHeartView_parentHeight, dp2px(120));
            pointColor = typedArray.getColor(R.styleable.BraceCusHeartView_pointColor, Color.WHITE);
            recfColor = typedArray.getColor(R.styleable.BraceCusHeartView_recfColor, 0);
            heartLineColor = typedArray.getColor(R.styleable.BraceCusHeartView_heartLineColor, Color.WHITE);
            timeStroke = typedArray.getDimensionPixelSize(R.styleable.BraceCusHeartView_timeStrokeHeart, sp2px(10));
            radioStroke = typedArray.getDimensionPixelSize(R.styleable.BraceCusHeartView_radioStrokeHeart, MiscUtil.dipToPx(context, 2));
            typedArray.recycle();
        }
        initPaint(context);
    }
    ...
    private void initPaint(Context mContext) {
        ...
        linPain.setStrokeWidth(MiscUtil.dipToPx(mContext, 1.5f));
    ...
    private void drawEmptyTxt(Canvas canvas) {
        if (rateDataList == null || rateDataList.size() <= 0) {
            canvas.drawText(getResources().getString(R.string.nodata), width / 2 - MiscUtil.getTextWidth(emptyPaint,getResources().getString(R.string.nodata))/2, -getHeight() / 2, emptyPaint);
        }
    }
